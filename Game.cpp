...
std::vector<Move> MigrationGame::getMovesStatic(int p, const std::vector<std::vector<int>>& b, int n) {
    ...
    // Strict Rules: P1 moves UP (y-1), P2 moves RIGHT (x+1)
    int dx = (p == 2) ? 1 : 0;
    int dy = (p == 1) ? -1 : 0;
        ...
        for(int y=0; y<n; y++) {
            if(b[x][y] == p) {
                ...
                if(nx>=0 && ny>=0 && nx<n && ny<n && b[nx][ny] == 0) {
                    moves.push_back({x,y,nx,ny, p}); // Store 'p' in move
                }
            }
        }
    }
...
void MigrationGame::makeMove(int x1, int y1, int x2, int y2) {
    ...
    // [NEW] Record History BEFORE clearing the old cell
...
void MigrationGame::runBot() {
    ...
    if(m.x1 != -1) {
        // Operator overloading handles the print format now
...
void MigrationGame::saveGame(std::string filename) {
    ...
    // 2. Write Readable Board State (Visual)
    f << "=== CURRENT BOARD ===\n";
    f << "  ";
    for(int x=0; x<n; x++) f << x << " "; // X-axis labels
    f << "\n";
    
    // Note: To visualize correctly (0,0 bottom left usually), we iterate Y normally (Top-Down in array)
    // Board[x][y] means x is col, y is row.
    for(int y=0; y<n; y++) {
        f << y << " "; // Y-axis label
        for(int x=0; x<n; x++) {
            int c = board[x][y];
    ...
    // 3. Write Raw Data (Hidden at bottom if needed for loading later)
    f << "\n=== RAW DATA ===\n";
...
void GameTests::runTests() {
    ...
    MigrationGame g(8, 1);
    Move m = {0,0,1,1,1}; // Added Owner ID
