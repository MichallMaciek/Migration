...
// [Interface/abstract class -> 1 pt (interface)]
interface IGameLogger {
    void log(String msg);
}

// [Enumeration -> 1 pt (enum)]
enum GameState {
    PLAYING,
    ANIMATING,
    GAME_OVER
...
// [Generic class/method -> 3 pt (class MyClass<T>)]
class MoveLogger<T> implements IGameLogger {
    
    // [Collections -> 1 pt (ArrayList)]
    private ArrayList<T> history = new ArrayList<>();

    // [Polymorphism -> 7 pt (Interface implementation)]
    @Override
    ...
    // [Read and write to file -> 1 pt (FileWriter/BufferedWriter)]
    public void saveToFile(String filename) {
        try (FileWriter fw = new FileWriter(filename);
            ...
            // [Error handling -> 2 pt (exceptions)]
...
// [Inheritance -> 2 pt (extends JFrame)]
public class Migration extends JFrame {
    ...
    public Migration() {
        ...
        String[] diffOptions = {"Easy (Depth 1)", "Medium (Depth 5)", "Hard (Depth 8)"};
        ...
        int depth = (diffChoice == 0) ? 1 : (diffChoice == 2 ? 8 : 5);
        ...
        // [Destructor -> 1 pt (close file/resources)]
        ...
        boardPanel = new JPanel() {
            // [Overriden method -> 1 pt (@Override)]
            @Override
    ...
    private void triggerBot() {
        // [Parallel programming -> 3 pt (Thread)]
        new Thread(() -> {
            int[] m = getBotMove();
            if (m != null && m[0] != -1) {
