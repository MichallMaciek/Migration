...
// [Generic class/method -> 3 pt (template <typename T>)]
template <typename T>
class Logger { public: static void log(T msg) { std::cout << "[LOG]: " << msg << std::endl; } };

// [Enumeration -> 1 pt (enum class)]
enum class PlayerID {
    NONE = 0,
    P1 = 1,
    P2 = 2
...
class MigrationGame {
private:
    // [Encapsulation-> 1 pt (private and protected)]
    int n;
    PlayerID currentPlayer; 
    std::vector<std::vector<int>> board;
    std::vector<Move> history;
    
    // [Dynamic allocation -> 2 pt (using new keyword or smart pointers)]
